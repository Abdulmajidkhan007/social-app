...
export default function FollowersScreen() {
  ...
  useEffect(() => {
    if (!userId) return;
    api.users.getFollowers(userId).then((data) => {
      setUsers(data);
      setLoading(false);
    });
  }, [userId]);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          !loading ? (
            <EmptyState icon="people-outline" title="No followers yet" subtitle="When people follow this account, they'll show up here" />
          ) : null
        }
        renderItem={({ item }) => (
